...
public class LecturasCaracteresIndividuales implements ProcessInterface {
    ...
    @Override
    public void init() {
        tabla_caracteristicas = new Mat(NUMERO_CLASES * MUESTRAS_POR_CLASE, NUMERO_CARACTERISTICAS, CvType.CV_64FC1);
        binaria1 = new Mat();
        binaria2 = new Mat();
...
//Leer numero
    ...
    void crearTabla() {
        double datosEntrenamiento[][] = new double[][]{
                new double[]{0.5757916569709778, 0.8068438172340393,
                        0.6094995737075806, 0.6842694878578186, 0, 0.6750765442848206, 0.573646605014801, 0.814811110496521, 0.6094995737075806},
                new double[]{0.5408163070678711, 0.04897959157824516, 0.8428571224212646, 0.79795902967453, 0.7795917987823486, 0.9938775897026062, 1, 0.995918333530426},
    ...
    boolean localizarCaracter(Rect digit_rect) {
        ...
        for (int c = 0; c < contornos.size(); c++) {
            Rect bb = Imgproc.boundingRect(contornos.get(c)); // Comprobar tamaño
            if (bb.width < anchura_minima || bb.height < altura_minima) continue;
            // Descartar proximos al borde
            if (bb.x < 2 || bb.y < 2)
                continue;
            if (binaria1.width() - (bb.x + bb.width) < 3 || binaria1.height() - (bb.y + bb.width) < 3)
                continue;
        ...
        if (max_area < 0) return false; //No se ha detectado objeto valido else return true;
        else return true;
    }
...
// Buscamos la fila de la tabla que mas se parece
        ...
        nmin = nmin % 10; // A partir de la fila determinamos el numero return nmin;
        return nmin;
    }
    ...
    public Mat caracteristicas(Mat recorteDigito) {
        ...
        recorteDigito.convertTo(chardouble, CvType.CV_64FC1); //Calculamos vector de caracteristicas
